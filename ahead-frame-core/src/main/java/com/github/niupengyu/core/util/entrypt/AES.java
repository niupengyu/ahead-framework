...
public class AES {
	...
	static void method2(String str) throws Exception {
		...
		logger.info("method2-加密：" + Arrays.toString(encrypt));
		cipher.init(Cipher.DECRYPT_MODE, secretKey);//使用解密模式初始化 密钥
		byte[] decrypt = cipher.doFinal(encrypt);
		logger.info("method2-解密后：" + new String(decrypt));
	...
	static void method3(String str) throws Exception {
		...
		logger.info("method3-加密：" + Arrays.toString(encrypt));
		cipher.init(Cipher.DECRYPT_MODE, secretKey, new IvParameterSpec(getIV()));//使用解密模式初始化 密钥
		byte[] decrypt = cipher.doFinal(encrypt);
		logger.info("method3-解密后：" + new String(decrypt));
	...
	static void method4(String str) throws Exception {
		...
		logger.info("method4-加密：" + Arrays.toString(encrypt));
		cipher.init(Cipher.DECRYPT_MODE, secretKey, new IvParameterSpec(getIV()));//使用解密模式初始化 密钥
		byte[] decrypt = cipher.doFinal(encrypt);
		
		logger.info("method4-解密后：" + new String(decrypt));
